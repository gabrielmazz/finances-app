...
Quando um componente visual compartilhado precisa bloquear eventos de ponteiro, prefira `pointer-events-none` em NativeWind ou `style.pointerEvents`; não encaminhe `pointerEvents` como prop a elementos DOM. Use `boxShadow` CSS para sombras dinâmicas no Web e mantenha os props `shadow*` apenas nas plataformas nativas. Animações do `Skeleton` usam o driver JS no Web e o driver nativo em Android/iOS.
...
| `uiverse/assistant/` | Assistente Lumus | `assistant-cards`, `assistant-composer-frame`, `assistant-route-boundary` |
...
| Componente | Uso |
|---|---|
| `button/` | Botões com variantes, spinner de loading e suporte a ícones; ações primárias sólidas usam `lumus-accent` (`#FACC15`) com texto e ícones brancos nos dois temas |
...
| `components/web/navigation/StaggeredMenu.jsx` / `.css` | Componente React DOM adaptado para a navegação Web: painel desktop com variante navy/amarelo no dark mode e white/slate com camadas amarelo suave no light mode, recortado a uma rail fixa de 68px quando fechado e expandido por animação contínua para mostrar as seções Home/Controle/Config e o perfil do usuário autenticado; mantém foco visível, Escape/clique externo, posicionamento explícito conforme `position` e variante de movimento reduzido |
...
| `components/uiverse/assistant/assistant-composer-frame.web.tsx` / `.native.tsx` | Moldura animada do compositor: pacote `border-beam` no Web; o mobile usa um traço SVG animado pelo Reanimated instalado, apenas com foco e respeitando movimento reduzido |
...
graph LR
    ...
    CADC[category-analysis-bank-donut-chart.tsx] --> CAS
...
- [[Análise por Categoria]] — Android/iOS usam `components/ui/tabs` e a Web usa `Tabs` do Mantine para alternar o relatório entre gastos e ganhos sem recarregar os dados; o gráfico de distribuição Web usa `category-analysis-bank-donut-chart` em Expo DOM. Os blocos de evolução, distribuição e últimas movimentações seguem o cabeçalho em caixa alta da Home e não usam card externo nem ícone decorativo de seção
...
- `nativewind@4.2.1`, `tailwindcss@3.4.18`, `@gluestack-ui/core@3.0.12`, `@gluestack-ui/utils@3.0.12`, `@react-stately/color@3.9.2` e `@react-stately/utils@3.10.8` ficam em versões exatas para conservar o toolchain compatível com Expo SDK 57
...
- Os componentes gerados usam `cssInterop()` do NativeWind 4. O patch de `react-native-css-interop@0.2.1` remove somente o mapeamento legado de `SafeAreaView` incompatível com a versão atual do React Native e é reaplicado no `postinstall`.
- `nativewind-env.d.ts` complementa os tipos dos submódulos concretos do React Native 0.86 para que `className` continue tipado em telas Web que usam `View`, `Text`, `Image`, `ScrollView` e `KeyboardAvoidingView` diretamente. Assets SVG/PNG/JPG são referenciados pela mesma entrada global.
...
- As superfícies Web de conteúdo devem usar os tokens `sheet`, `contentFrame` e `contentPadding` de `WEB_DASHBOARD_CLASS_NAMES`, mantendo gutter responsivo de `24px`/`32px` e largura máxima de `1180px`. Telas canônicas carregadas por uma rota Web usam as variantes `webSheet`, `webContentFrame` e `webContentPadding` para preservar o layout mobile sem duplicar a geometria.
...
- Em telas `.web.tsx`, use o `Text` de `react-native` quando houver arrays de `StyleSheet`, `numberOfLines` ou props de acessibilidade. O `components/ui/text/index.web.tsx` renderiza um `span` DOM direto e repassa props sem a normalização do React Native Web. O `components/ui/heading/index.web.tsx` mapeia `Heading` para elementos semânticos `<h1>`–`<h6>`; não passe `accessibilityRole` nesses títulos, pois o elemento HTML já fornece o papel de cabeçalho e a prop pode chegar sem normalização ao DOM.
- `tabs/` mantém a API composta `Tabs`/`TabsList`/`TabsTrigger`/`TabsTriggerText`/`TabsIndicator` e usa o indicador no thread de UI para as telas nativas; o código é compatível com a linha estável instalada e não deve reintroduzir imports do creator de Tabs do Gluestack 5. Nas telas nativas de previsão, análise e investimentos, a composição deve ficar dentro de um card `notTintedCardClassName`; o indicador preenchido usa o amarelo ativo do sistema e o item selecionado mantém texto/ícone brancos. A composição Web de `CategoryAnalysisScreen.web.tsx` usa `Tabs` do Mantine com a mesma variante de classes, variáveis e estilos do extrato bancário: amarelo `#FACC15`, texto/ícone brancos e sem sombra na tab ativa, foco/hover e desabilitação contextual; tabs ficam no topo e o seletor de categoria abaixo. O gráfico de distribuição Web usa `category-analysis-bank-donut-chart.tsx` em Expo DOM, sem alterar a API mobile.
...
- O compositor de [[Assistente Lumus]] usa `ASSISTANT_CLASS_NAMES`: texto, áudio e envio formam uma superfície agrupada; o foco amarelo fica somente no campo de texto. A tela mobile usa `PromptInputTextarea` do Gluestack; a Web usa `Textarea` do Mantine e os contratos `MANTINE_ASSISTANT_TEXTAREA_CLASS_NAMES`. O texto digitado mantém o contraste do tema e o placeholder Web usa `#5A6A7F` em ambos os temas, resolvido pelo token `textPlaceholder` no adaptador Mantine. Enter envia a mensagem; Shift+Enter mantém quebras de linha na Web e o teclado mobile usa a ação **send**. Controles de ícone respeitam `h-touch`/`w-touch`. A tela mobile preserva a fachada legada somente para tema e geometria do hero/teclado; a tela Web importa os contratos diretamente. Valores calculados de hero, insets ou teclado e props imperativas de gráficos são as únicas exceções para `style`.
...
- As preferências de [[Assistente Lumus]] são abertas pelo `drawer/` à direita, em vez de ocupar o histórico do chat. O mobile usa as cores resolvidas da fachada legada; a variante Web resolve a paleta diretamente de `LUMUS_RUNTIME_COLORS`. Revogar consentimento na Web exige confirmação antes de abortar a IA e limpar a conversa.
...
- `useScreenStyles()` mantém as variantes Web de `fieldContainerClassName` e `textareaContainerClassName`; inputs textuais/numéricos, selects e textareas compartilham borda e anel amarelo vivo `lumus-accent` no foco por clique e teclado. No Android/iOS, os campos mantêm borda neutra em repouso e mostram contorno de 2 px no mesmo amarelo ao receber foco; cards mantêm apenas o contorno neutro. `TagsInput`/`NumberInput` Mantine seguem o destaque Web pelo adaptador, erros continuam vermelhos e o login Web não suprime o anel. A ação primária sólida mantém o fundo `lumus-accent` (`#FACC15`) e usa texto e ícones brancos, com os estados hover/pressed do acento.
