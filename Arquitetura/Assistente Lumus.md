...
6. Quando vários rascunhos precisam do mesmo banco ou categoria, a pergunta oferece **Aplicar também aos semelhantes**.
   ...
   Rascunhos preparados pela mesma resposta aparecem em uma única entrada do chat, com um cartão visível por vez. A paginação numerada, a posição e a contagem de ações concluídas ficam em uma faixa fixa **abaixo do histórico rolável e imediatamente acima do compositor**, seguindo a navegação numérica de [[Configurações]]. A seleção fica apenas na memória da tela; alternar páginas não prepara nem executa ações. Cada cartão conserva seu `clientActionId`, estado, edição, dependências e confirmação individual. Quando a edição está disponível, o lápis abre no próprio campo um controle correspondente ao tipo: texto, valor em reais convertido para centavos inteiros, número, data civil em `DD/MM/AAAA`, horário ou seletor de opção. Web e mobile compartilham `AssistantInlineField` e os tokens de formulário de `AddRegisterExpensesScreen.web.tsx` para label, altura, borda, superfície, tipografia e foco; no mobile, Gluestack e os seletores nativos preservam a interação por plataforma. Banco, categoria e demais referências exibem rótulos usando handles opacos; opções indisponíveis não aceitam texto livre. Salvar a edição atualiza somente o rascunho por `financeCommandService`; a gravação continua dependendo da confirmação individual do cartão. Com valores ocultos, o editor não revela o valor atual e permite informar um substituto. Os campos do cartão não têm divisores horizontais; o espaçamento vertical separa os valores. Pedidos posteriores criam grupos separados, cada qual com sua linha de paginação. Enquanto há campo obrigatório pendente, a pergunta ativa fica abaixo dos cartões e avisos como última entrada do histórico; a faixa seleciona automaticamente a ação perguntada e impede trocar de página naquele grupo até responder. Campos de nome são respondidos individualmente para cada registro, sem aplicação em lote. Depois da última resposta, o grupo de cartões passa ao fim do histórico para revisão e confirmação individual, abaixo das perguntas respondidas. A ordenação visual não duplica mensagens nem altera IDs ou a execução financeira.
