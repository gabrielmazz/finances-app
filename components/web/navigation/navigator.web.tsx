...
const getWebLink = (option: NavigatorOption) => {
	if (option.id === 'home-start') return `${APP_ROUTE_PATHS.home}?tab=0`;
	if (option.id === 'settings') return `${APP_ROUTE_PATHS.home}?tab=2`;
	return option.matchPaths?.[0] ?? `${APP_ROUTE_PATHS.home}?tab=0`;
};
