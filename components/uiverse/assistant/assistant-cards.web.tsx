...
const AssistantChart = ({ report, hideValues, isDarkMode }: { report: AssistantReport; hideValues: boolean; isDarkMode: boolean }) => {
	...
	const data = report.chart.points.map(point => ({
		label: point.label,
		value: point.value,
		color: point.color ?? 'yellow.5',
	}));
	return (
		<MantineProvider forceColorScheme={isDarkMode ? 'dark' : 'light'}>
			<div
				role="img"
				aria-label={`Gráfico do relatório ${report.title}`}
				style={{ minHeight: 170, width: '100%', display: 'flex', justifyContent: 'center', overflow: 'hidden' }}
			>
				{report.chart.kind === 'bar' ? (
					<BarChart
						w={chartWidth}
						h={140}
						data={data}
						dataKey="label"
						series={[{ name: 'value', label: 'Valor', color: 'yellow.5' }]}
						withTooltip
						withYAxis={false}
						tickLine="none"
						gridAxis="none"
				...
				{report.chart.kind === 'donut' ? (
					<DonutChart
						data={data.map(point => ({ name: point.label, value: point.value, color: point.color }))}
						size={150}
						thickness={38}
						withTooltip
						withLabels
						labelsType="value"
						chartLabel="Relatório"
					/>
				) : null}
			</div>
		</MantineProvider>
