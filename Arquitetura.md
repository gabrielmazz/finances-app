...
- Correção do bootstrap Firebase Android em 2026-08-20: `utils/firebaseRuntime.ts` captura cada `EXPO_PUBLIC_*` por acesso direto para o Metro incorporar a configuração no bundle, e releases locais com credenciais completas inferem produção sem depender de uma variável de alvo ausente. Somente `development` usa o Emulator; `preview` passa a ser APK instalável contra produção com App Check debug, enquanto `production`/`production-apk` mantêm Play Integrity. O preview e os perfis de produção exigem `GOOGLE_SERVICES_JSON`; a ação **Run Dev Client** inicia/semeia a Suite local e aplica `adb reverse`. Isso elimina a exceção anterior à montagem do Expo Router que mantinha `splash-icon.png` indefinidamente. Vault alinhado em [[Firebase Config]], [[Autenticação]] e [[Navegação]].
