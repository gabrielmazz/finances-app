...
Expo Router (file-based routing) — app/
    ↓
Screens — screens/mobile/ + screens/web/
    ↓
...
screens/mobile/ → Telas canônicas para Android/iOS e fluxos compartilhados sem composição Web dedicada
screens/web/    → Composições exclusivas do navegador e adaptadores Web das telas divergentes
...
- Acompanhamento do Lumus IA em 2026-09-25: `LumusAssistantContext` expõe a etapa real de consulta de dados, interpretação via gateway, preparação determinística de cartões e geração de relatório. `AssistantActivityTrace` apresenta etapa atual e etapas concluídas na Web e no mobile; não simula nem expõe pensamentos privados do modelo, argumentos, texto do pedido ou dados financeiros. O estado é apenas em memória e some ao concluir ou limpar a sessão. Typecheck e export Web/Android/iOS passaram; lint de estilos continua limitado às pendências registradas em `ConfigurationsScreen.web.tsx` e ao baseline de `useScreenStyles` (55/54). Não houve inspeção visual autenticada em navegador ou aparelho. Vault alinhado em [[Assistente Lumus]] e [[Auditoria de Design]].
- Feixe do compositor do Lumus IA em 2026-09-25: `components/uiverse/assistant/assistant-composer-frame.web.tsx` usa `border-beam@1.4.1` com paleta dourada, intensidade moderada e animação apenas durante o foco; `assistant-composer-frame.native.tsx` percorre a borda com SVG/Reanimated já instalados. Os dois respeitam movimento reduzido. Typecheck, export Web, Android e iOS, e `git diff --check` passaram; a inspeção interativa em navegador e aparelho ainda falta. O pacote npm é somente React DOM; o mobile foi mantido sem dependência nativa nova. Vault alinhado em [[Assistente Lumus]], [[Componentes UI]] e [[Auditoria de Design]].
- Estilo dos inputs editáveis nos cartões do Lumus IA em 2026-09-25: os cartões Web e mobile usam o mesmo `AssistantInlineField` e contrato de `design-system/assistant.ts`, baseado em `LUMUS_FORM_CLASS_NAMES` de `AddRegisterExpensesScreen.web.tsx` para labels, altura, borda, superfície, tipografia e foco. O Android mantém os controles Gluestack e seletores nativos de data/horário. Texto, moeda, número, data, horário e opções permanecem apropriados ao tipo; editar só atualiza o rascunho. Typecheck e exports Web/Android passaram; `git diff --check` passou; a conferência interativa em navegador/aparelho ainda falta. Vault alinhado em [[Assistente Lumus]] e [[Auditoria de Design]].
- Remoção dos divisores horizontais entre campos nos cartões do Lumus IA em 2026-09-25: `AssistantInlineField` deixa de aplicar a borda inferior e o padding de linha; `cardBody` preserva a separação com espaçamento vertical. A regra visual compartilhada foi removida de `design-system/assistant.ts` e documentada em [[Assistente Lumus]] e [[Auditoria de Design]].
- Edição dos campos nos cartões do Lumus IA em 2026-09-25: o lápis abre o controle no próprio campo, com entrada de texto/textarea, moeda e número, calendário civil em `DD/MM/AAAA`, seletor de horário ou lista para banco/categoria e outras escolhas. O componente compartilhado `components/uiverse/assistant/assistant-inline-field.tsx` é usado nos cartões Web e mobile. A edição valida e atualiza apenas o rascunho por `financeCommandService`; confirmar continua sendo uma ação separada. Valores monetários ficam em branco ao abrir a edição quando a privacidade está ocultando-os. Passaram 10 suítes do assistente (91 testes), typecheck, export Web, export Android e `git diff --check`. O lint de estilos continua falhando somente pelas pendências de `ConfigurationsScreen.web.tsx` e pelo limite global de `useScreenStyles` (55/54); a conferência visual em navegador/dispositivo ainda falta. Vault alinhado em [[Assistente Lumus]] e [[Auditoria de Design]].
- Posição da paginação do Lumus IA em 2026-09-25: os botões de página de cada grupo de ações saíram de dentro do cartão e passaram para uma faixa fixa logo abaixo do histórico rolável e acima do compositor, em Web e mobile. O cartão continua no histórico; a seleção é mantida apenas em memória, segue a pergunta ativa e não cria nem executa operações. Seleções de grupos removidos são limpas junto com a conversa. Componente em `components/uiverse/assistant/assistant-draft-pages.tsx` e posições controladas pelas duas telas do assistente. Passaram 11 suítes do assistente (93 testes), typecheck, export Web e Android, e `git diff --check`. Falta validação visual autenticada no navegador e em aparelho; os exports confirmam o empacotamento, não a execução interativa. Vault alinhado em [[Assistente Lumus]] e [[Configurações]].
- Perguntas e paginação do Lumus IA em 2026-09-25: em respostas com várias ações, todo campo obrigatório é pedido na última entrada visível do chat, abaixo dos cartões e de avisos posteriores, com identificação `Ação N de M`; o paginador acompanha a ação perguntada. Um pedido que prepara quatro despesas sem nome exige quatro respostas distintas, uma por `clientActionId`, sem compartilhar nomes. Depois da última resposta, o grupo volta ao fim do histórico para mostrar a revisão e os botões individuais após a configuração. As 11 suítes do assistente passaram (93 testes), além de typecheck, export Web, export Android e `git diff --check`; o lint de estilos segue bloqueado por pendências já existentes em Configurações. Falta conferência visual autenticada no navegador e em um development client Android. Vault alinhado em [[Assistente Lumus]].
- Regras de gravação do Lumus IA em 2026-09-25: um log Web local mostrou `permission-denied` em `BatchGetDocuments` para o ID determinístico ainda ausente do pagamento obrigatório. `firestore.rules` usava `resource.data.personId` em `allow read`, impedindo a leitura prévia de idempotência e, portanto, o commit. As coleções legadas usadas pelo assistente agora permitem `get` somente de documento ausente para usuário autenticado; documentos existentes conservam a autorização por `personId`, e escrita/listagem permanecem iguais. `financeCommandService` distingue essa negativa do resultado incerto de transporte. Teste de transação no Firestore Emulator reproduziu a falha antes da correção e passou depois, incluindo bloqueio de leitura por outra conta; 11 suítes do assistente (91 testes), typecheck de app/backend, export Web e `git diff --check` passaram. Lint de estilos segue com dívida existente em Configurações. As regras corrigidas foram carregadas no Emulator já aberto para `demo-lumus-financas` sem limpar dados, e um `get` de documento ausente foi verificado; falta repetir o pagamento na sessão Web autenticada. Regras remotas não foram implantadas. Vault alinhado em [[Assistente Lumus]] e [[Firebase Config]].
- Confirmação de pagamentos no Lumus IA em 2026-09-25: a entrada de revisão do grupo agora acompanha a pergunta de configuração mais recente, de modo que a escolha da obrigação e da conta precede o botão de confirmação. A transição para `confirming` também sincroniza imediatamente o rascunho lido por `executeDraft`, impedindo que um toque rápido em **Confirmar agora** seja ignorado por estado anterior. O cartão continua individual, com escrita exclusivamente após confirmação e validação de `financeCommandService`; a paginação conserva os mesmos IDs. As 11 suítes do assistente passaram (90 testes), assim como typecheck, export Web e `git diff --check`; lint de estilos segue bloqueado pela dívida preexistente em Configurações. Ainda falta execução autenticada na Web local com o Emulator Suite. Vault alinhado em [[Assistente Lumus]].
- Seleção de obrigações pendentes no Lumus IA em 2026-09-25: pedidos amplos para pagar contas/gastos obrigatórios ou receber ganhos/receitas obrigatórios deixam de cair apenas no relatório e preparam localmente um cartão com pergunta de escolha. O catálogo mostra somente itens próprios ainda pendentes no ciclo atual e com parcelas disponíveis; data civil de hoje e banco/caixa são revisados antes da confirmação individual. `financeCommandService` confere dentro da transação se o mesmo `clientActionId` já criou o lançamento, evitando incrementar parcelas ao repetir o cartão, e mantém os bloqueios de snapshot e ciclo concluído. Três suítes específicas passaram (47 testes), assim como typecheck, export Web, export Android e `git diff --check`; lint de estilos continua bloqueado pelas pendências já existentes em Configurações e na tela Web do assistente. Sem gravação no Firestore remoto ou execução autenticada em navegador/aparelho. Vault alinhado em [[Assistente Lumus]].
- Resiliência do `npm run dev:local` em 2026-09-25: o launcher supervisiona o processo Expo e tenta reiniciá-lo após uma saída inesperada, mantendo a mesma Suite Firebase ativa e sem repetir o seed. `Ctrl+C` encerra o Expo e os emuladores iniciados nessa execução. `bash -n` e `git diff --check` dos arquivos alterados passaram; suíte de testes não executada. Documentação alinhada em [[Firebase Config]] e README.
- Paginação dos cartões do Lumus IA em 2026-09-25: cada resposta com ações cria uma única entrada de rascunhos no chat; Web e mobile mostram um cartão por vez com seleção numérica e contagem de concluídos. A seleção é local à tela e mantém `clientActionId`, dependências e confirmação individual de cada ação, sem escrita ao trocar de página. Contrato visual em `design-system/assistant.ts` e componente compartilhado em `components/uiverse/assistant/assistant-draft-pages.tsx`. Typecheck, export Web, export Android e `git diff --check` passaram; o lint de estilos permanece bloqueado apenas pelas pendências preexistentes em `ConfigurationsScreen.web.tsx` e pelo limite global de `useScreenStyles` (55/54). A interface ainda precisa de inspeção visual autenticada no navegador/aparelho. Vault alinhado em [[Assistente Lumus]] e [[Configurações]].
- Correção recorrente do wallpaper do Lumus IA Web em 2026-09-25: a revisão encontrou que a imagem do hero ainda dependia apenas das classes NativeWind, diferente de `AddRegisterExpensesScreen.web.tsx`, que fixa a imagem React Native com posicionamento e dimensões explícitos. `LumusAssistantScreen.web.tsx` agora replica essa estrutura de camadas e o wrapper relativo do cadastro, preservando o `Grainient` animado, `StrokeText`, `AnimatedContent`, wallpaper e ilustração próprios. Typecheck, export Web e `git diff --check` passaram; `npm run lint:styles` continua limitado às pendências existentes em `ConfigurationsScreen.web.tsx` e ao limite global de `useScreenStyles` (55/54). Não há navegador disponível nesta sessão para confirmar visualmente a rota autenticada. Vault alinhado em [[Versão Web]], [[Assistente Lumus]] e [[Componentes UI]].
- Compositor do Lumus IA Web em 2026-09-25: `LumusAssistantScreen.web.tsx` usa Mantine `Textarea` com tema sincronizado ao app; o foco amarelo fica no campo, não na superfície externa, e texto digitado/placeholder são brancos no tema escuro. A variante mobile segue com `PromptInputTextarea` do Gluestack. Typecheck e `git diff --check` passaram; `npm run lint:styles` continua limitado às pendências preexistentes em `ConfigurationsScreen.web.tsx` e ao limite global de `useScreenStyles` (55/54). Inspeção visual autenticada pendente. Vault alinhado em [[Assistente Lumus]], [[Componentes UI]], [[Versão Web]] e [[Auditoria de Design]].
- Balão de prompt do Lumus IA em 2026-09-25: Web e mobile compartilham `ASSISTANT_CLASS_NAMES.userBubble`, agora com `lumus-accent` no tema claro, `lumus-accent-dark` no escuro e texto `lumus-on-accent`. `npm run typecheck` e `git diff --check` passaram; `npm run lint:styles` continua limitado às pendências preexistentes de `ConfigurationsScreen.web.tsx` e ao limite global de `useScreenStyles` (55/54). Não houve inspeção visual autenticada em navegador ou aparelho. Vault alinhado em [[Assistente Lumus]] e [[Auditoria de Design]].
- Padronização visual do Lumus IA Web em 2026-09-25: `LumusAssistantScreen.web.tsx` voltou ao hero das telas do sistema, com wallpaper, título e ilustração do Lumus IA sobre painel arredondado. A coluna de sugestões foi removida para não ficar encoberta pela navegação; o botão de lâmpada junto à lixeira abre o modal de prompts. A variante Web continua independente da tela móvel. Vault alinhado em [[Assistente Lumus]] e [[Versão Web]].
- Análise por categoria em 2026-09-25: as tabs Web de `CategoryAnalysisScreen.web.tsx` reutilizam os contratos Mantine do extrato bancário; no mobile, as tabs de gastos/ganhos aparecem antes da categoria e texto/ícone selecionados ficam brancos. As superfícies das duas variantes foram alinhadas a cantos de 16px. A distribuição por conta Web usa `category-analysis-bank-donut-chart.tsx` com Mantine `DonutChart` em Expo DOM, valores em centavos e privacidade preservada. Typecheck, export Web e `git diff --check` passaram; `npm run lint:styles` continua limitado à pendência preexistente de `ConfigurationsScreen.web.tsx`/linha de base de `useScreenStyles`. Sem inspeção visual autenticada no navegador ou aparelho nesta execução. Vault alinhado em [[Análise por Categoria]], [[Componentes UI]], [[Componentes por Sistema]], [[Versão Web]] e [[Auditoria de Design]].
- Falha real do Lumus IA Web local reproduzida em 2026-09-25: o token App Check Debug e o template Remote Config ativo foram lidos com sucesso (`enabled=true`, `gemini-3.8-flash`), mas o modelo principal devolveu `500` por alta demanda e depois `429`; o classificador confundia `fetch-error` HTTP com falta de internet. O ciclo de ferramenta com modelo alternativo gratuito mostrou o bloqueio central: `functionResponse` com `role: function` recebeu `400`, e o mesmo resultado com `role: user` recebeu `200`. `firebase@12.19.0` corrige a serialização Web; o adaptador Android usa `generateContent` para evitar o papel antigo do RN Firebase e preserva o ID da chamada. O gateway faz no máximo uma tentativa com `gemini-3.5-flash-lite` após cota/indisponibilidade e avisa a pessoa. Passaram 43 suítes/257 testes, typecheck app/backend e export Web/Android; a execução no navegador da pessoa e em novo development client ainda não foi observada. Não houve gravação no Firestore remoto nem alteração de Remote Config. Vault alinhado em [[Assistente Lumus]] e [[Firebase Config]].
- Gráfico diário no extrato Web em 2026-09-24: `BankMovementsScreen.web.tsx` exibe `BankMovementsDailyAreaChart` acima da timeline, somando ganhos e despesas em centavos por cada dia do período selecionado e acompanhando os filtros ativos. Transferências e investimentos continuam excluídos; tema e privacidade usam os tokens/séries neutras. O mobile permanece inalterado. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` segue limitado à dívida preexistente de `ConfigurationsScreen.web.tsx` e ao limite global de `useScreenStyles`. Vault alinhado em [[Gerenciamento de Bancos]], [[Versão Web]], [[Componentes UI]] e [[Privacidade de Valores]].
- Auditoria do Lumus IA em 2026-09-24: `prepareAssistantActions()` passou a gerar IDs locais por cartão e remapear dependências para impedir que rótulos repetidos do modelo reutilizem um documento financeiro; o gateway responde também às chamadas de função excedentes sem executá-las. Remote Config usa padrões locais se a inicialização falhar, e o cartão mantém sucesso após commit mesmo se a atualização de dependentes falhar. Respostas assíncronas canceladas ou de outra conta não repovoam a sessão. Erros `404` de modelo/configuração recebem mensagens distintas. Os scripts de deploy Web forçam o alvo remoto e removem o token de debug do export. A lista oficial confirma `gemini-3.8-flash`. A suíte passou (41/248), assim como typecheck, typecheck do backend, export Web e bundle Android; não houve smoke test autenticado Web/Android nem leitura do template remoto ativo. Vault alinhado em [[Assistente Lumus]] e [[Firebase Config]].
- Bloqueio dos filtros do extrato bancário em 2026-09-24: Web e mobile mantêm datas, tipo, categorias e exportação PDF desativados até selecionar um banco; o seletor segue disponível e a rota de Caixa mantém os controles ativos. `npm run typecheck` e `git diff --check` passaram; `npm run lint:styles` segue apontando dívidas em `ConfigurationsScreen.web.tsx` e no limite global de `useScreenStyles`. Vault alinhado em [[Gerenciamento de Bancos]] e [[Versão Web]].
- Ícone do botão de baixar resumo PDF no extrato bancário em 2026-09-24: `BankMovementsScreen.web.tsx` e `BankMovementsScreen.tsx` mantêm o ícone branco nos dois temas. O fluxo de exportação e os demais estilos do botão permanecem inalterados.
- Ajuste do filtro de categorias do extrato Web em 2026-09-24: o placeholder `#505D74` fica à esquerda, centrado verticalmente e recuado; some após a primeira seleção. As categorias escolhidas recebem o mesmo recuo, com nome e X brancos; o menu usa `#FACC15` com nomes, contagens e ícones brancos. O X de limpar tudo sobrescreve o fundo inline do Mantine e permanece transparente. Vault alinhado em [[Gerenciamento de Bancos]], [[Versão Web]] e [[Auditoria de Design]].

- Complemento da auditoria do extrato bancário Web em 2026-09-24: quatro textos da timeline em `BankMovementsScreen.web.tsx` e dois textos de `bank-actionsheet-selector.web.tsx` trocaram `numberOfLines={1}` por `isTruncated`, pois o `Text` Gluestack Web é um `<span>` e encaminhava a prop desconhecida ao DOM. O truncamento de uma linha permanece; a variante nativa não foi alterada. `npm run typecheck` e `git diff --check` passaram. `npm run lint:styles` segue bloqueado pelas pendências preexistentes em Configurações Web e `useScreenStyles`. Vault alinhado em [[Gerenciamento de Bancos]] e [[Auditoria de Design]].
- Ajuste fino do filtro de tipo do extrato mobile em 2026-09-24: os três controles continuam com largura igual e agora usam `gap-1` para manter uma pequena separação visual entre os cards. Vault alinhado em [[Gerenciamento de Bancos]] e [[Auditoria de Design]].
- Correção da cor da tab selecionada no extrato Web em 2026-09-24: a variante Mantine específica de `BankMovementsScreen.web.tsx` sobrescreve o acento escuro do tema e mantém o fundo ativo em `#FACC15`, com texto e ícone brancos e sem sombra. Vault alinhado em [[Gerenciamento de Bancos]], [[Sistema de Temas]] e [[Versão Web]].
- Auditoria e correção do extrato bancário Web/mobile em 2026-09-24: `BankMovementsScreen.web.tsx` removeu a sombra da tab ativa, usa ícone/label brancos e placeholder `#505D74` no filtro de categorias com padding alinhado aos inputs; as duas variantes removeram o botão redundante de busca e o card de erro por ausência de banco, trocaram a terminologia visível de tag para categoria e preservaram a consulta automática. No mobile, os três controles de tipo ocupam toda a largura com uma pequena separação e exibem conteúdo branco no estado selecionado; estados acessíveis dos filtros e anúncio de erro foram revisados. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` segue bloqueado apenas por `ConfigurationsScreen.web.tsx` e pela linha de base global de `useScreenStyles`. Sem Chrome DevTools MCP ou dispositivo nativo nesta sessão. Vault alinhado em [[Gerenciamento de Bancos]], [[Versão Web]] e [[Auditoria de Design]].
- Padronização da Home em 2026-09-24: os blocos Web **Gastos por dia**, **Atividade no ano** e **Próximos compromissos** usam a largura integral de `scrollContent`, alinhada a bancos e movimentações, preservando os cards internos e os dados. A auditoria mobile corrigiu os nomes acessíveis dos ícones de ajuda e identificou os controles de banco, gráfico e expansão; referências e pendências estão em [[Exemplo Home Web]], [[Exemplo Home Mobile]] e [[Auditoria de Design]]. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` segue bloqueado apenas pela dívida já presente em Configurações Web e pela linha de base global de `useScreenStyles`.
- Correção dos estados de Mantine no extrato Web em 2026-09-24: `design-system/mantine.ts` deixou de passar pseudo-seletores inline para Tabs, TagsInput e o botão de limpeza; os estados ativos, selecionados, hover e foco usam `classNames` com variantes `data-*` e tokens semânticos, removendo o warning de `&[data-combobox-selected]`. A composição de categorias no extrato e a variante mobile permanecem conforme documentadas. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` continua bloqueado somente pela dívida já existente em `ConfigurationsScreen.web.tsx` e pela linha de base global de `useScreenStyles`. Vault alinhado em [[Sistema de Temas]].
- Filtro de categorias dos movimentos bancários Web em 2026-09-24: `BankMovementsScreen.web.tsx` usa o label **Categorias** e o mesmo contrato `fieldLabel` do formulário de despesas; o placeholder virou **Selecione alguma categoria**, centralizado e em cinza `slate-500`. As pílulas mantêm o amarelo e exibem texto branco, e o X para limpar todas as categorias fica branco e sem fundo. Seleção por IDs, filtragem contextual, tela nativa e dados financeiros permanecem inalterados. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` ainda aponta as alterações preexistentes em `ConfigurationsScreen.web.tsx` e o limite global de consumidores de `useScreenStyles`. Vault alinhado em [[Gerenciamento de Bancos]] e [[Versão Web]].
- Timeline de movimentações bancárias Web em 2026-09-24: `BankMovementsScreen.web.tsx` agora usa a hierarquia e os contratos da timeline compartilhada com despesas obrigatórias e investimentos: trilho, ícone, identidade, valor/data à direita e detalhes expansíveis com `AnimatedContent`/`Grainient`. Paletas por tipo, filtros, metadados e ações financeiras foram mantidos; a composição mobile não mudou. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` continua bloqueado somente pelas ocorrências preexistentes em `ConfigurationsScreen.web.tsx` e pelo limite global de consumidores de `useScreenStyles`. Sem sessão autenticada disponível para conferir movimentos reais no navegador. Vault alinhado em [[Gerenciamento de Bancos]], [[Versão Web]] e [[Auditoria de Design]].
- Card bancário colorido removido do extrato mobile em 2026-09-24: `BankMovementsScreen.tsx` inicia pelos filtros, sem repetir nome do banco, período e saldos em um card. O resumo filtrado continua na tela; os totais gerais permanecem disponíveis no PDF, e a timeline e os dados financeiros não mudaram. `npm run typecheck` e `git diff --check` passaram. `npm run lint:styles` segue bloqueado pelas ocorrências preexistentes em `ConfigurationsScreen.web.tsx` e pelo limite global de consumidores de `useScreenStyles`. Vault alinhado em [[Gerenciamento de Bancos]] e [[Auditoria de Design]].
- Seletor de banco no extrato mobile em 2026-09-24: `BankMovementsScreen.tsx` usa `BankActionsheetSelector` antes do período, com bancos ativos, ícone e cor, seguindo os formulários mobile e o extrato Web. O banco vindo da rota continua pré-selecionado; sem contexto, a pessoa pode escolhê-lo na própria tela. A seleção recarrega o extrato, atualiza o título e limpa a tag selecionada; período, tipo de movimento, visão de Caixa e dados do PDF são preservados. `npm run typecheck` e `git diff --check` passaram. `npm run lint:styles` aponta dívidas sem relação com esta tela em `design-system/mantine.ts`, `design-system/tokens.ts`, `ConfigurationsScreen.web.tsx` e no limite global de `useScreenStyles`. Vault alinhado em [[Gerenciamento de Bancos]] e [[Auditoria de Design]].
- Restauração do navegador lateral em Configurações Web em 2026-09-23: `ConfigurationsScreen.web.tsx` voltou a montar o `Navigator` compartilhado com `defaultValue={2}`, mantendo a rail desktop e a navegação compacta responsiva acessíveis nessa tela. O fluxo e a apresentação Android/iOS não mudaram. `npm run typecheck` e `git diff --check` passaram; `npm run lint:styles` continua falhando pela dívida já existente nessa tela e pela linha de base de `useScreenStyles`. Vault alinhado em [[Configurações]], [[Navegação]] e [[Versão Web]].
- Teste seguro do [[Assistente Lumus]] em desenvolvimento em 2026-09-21: `EXPO_PUBLIC_FIREBASE_TARGET=emulator` continua isolando Auth, Firestore e Functions em `demo-lumus-financas`, mas AI Logic e Remote Config — produtos sem emulador local — usam uma ponte restrita ao app remoto `finances-app-e8685`, sempre com App Check Debug. Web cria `LUMUS_ASSISTANT_DEVELOPMENT`; Android usa o app nativo do `google-services.json` e não envia o token incompatível do Auth Emulator ao projeto remoto. A tela mostra o modo híbrido, oferece **Tentar novamente** em ambas as plataformas e nunca libera o compositor sem preflight do App Check. O perfil EAS `development` agora também exige `GOOGLE_SERVICES_JSON`; Expo Go permanece sem IA. Vault alinhado em [[Firebase Config]] e [[Assistente Lumus]].
- Firebase AI Logic/Remote Config auditado em 2026-09-21: o projeto remoto `finances-app-e8685` e os apps Web/Android foram confirmados pela Firebase CLI; `google-services.json` local corresponde a `com.gabrielmazz.lumusfinances` e permanece ignorado. O template versionado `remote_config.json` foi publicado como Remote Config v1 com kill switch, `gemini-3.8-flash` e limites 12/20/8/10. O Console confirma plano Spark, Gemini Developer API ativa, Agent Platform desativada, providers reCAPTCHA Enterprise/Play Integrity registrados e App Check **Registrado (aplicado)** nos dois apps para AI Logic; Firestore não recebeu enforcement. Web e Android usam o mesmo fallback, removem `temperature`/`topP` incompatíveis com Gemini 3.x e rejeitam nomes antigos, preview, experimental, `-latest` ou modelos dedicados à geração de imagem/áudio como seleção conversacional. Testes Web cobrem App Check ausente/configurado, autenticação, Remote Config disponível/indisponível, fallback de modelo, resposta e function calling que produz somente rascunhos. A lista de domínios autorizados do Authentication não carregou no Console, o EAS CLI não está autenticado e os smoke tests reais Web/Android permanecem pendentes. Vault alinhado em [[Firebase Config]] e [[Assistente Lumus]].

- Ajuste de composição do cadastro de investimento Web em 2026-09-19: `screens/web/AddFinanceScreen.web.tsx` removeu os sublabels auxiliares adicionais, restaurou o trigger de banco no padrão visual de `AddRegisterExpensesScreen.web.tsx` e voltou a empilhar **Banco vinculado** acima de **Saldo disponível do banco**. Validação financeira, foco, bloqueio por saldo e variante mobile permanecem inalterados. Vault alinhado em [[Investimentos]], [[Versão Web]] e [[Componentes UI]].

- Home Web sem cards externos em 2026-09-19: `screens/web/HomeScreen.web.tsx` removeu somente as molduras externas de **Gastos por dia**, **Atividade no ano** e **Próximos compromissos**; labels, gráficos, estados de loading/erro e os cards internos das colunas de compromissos permanecem. Os contratos de layout em `design-system/web-dashboard.ts` mantêm o padding sem `border`/`rounded-section`. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` continua limitado às dívidas existentes do dashboard/configurações e à linha de base de `useScreenStyles`. Vault alinhado em [[Dashboard Home]], [[Versão Web]] e [[Auditoria de Design]].

- Padronização dos labels de seção Web em 2026-09-19: `WEB_DASHBOARD_CLASS_NAMES.sectionHeadingText` passou a reproduzir a tipografia de **Calendário de Vencimentos** (`text-lg font-bold uppercase tracking-widest`). `HomeScreen.web.tsx` consome o contrato por `webDashboardClassNames` para **Meus Bancos e Dinheiro**, **Investimentos**, **Gastos por dia**, **Atividade no ano**, **Próximos compromissos** e **Últimas Movimentações**; `FinancialListScreen.web.tsx` usa o mesmo contrato em **Rentabilidade por período**, e `date-calendar.web.tsx` tornou a referência explícita. Conteúdo, tabs, dados, cálculos em centavos, privacidade, Firebase, navegação e variante mobile permanecem inalterados. `npm run typecheck`, `npm run typecheck:backend`, `npm run test -- --runInBand`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` e o gate `npm run check` continuam bloqueados somente pela dívida preexistente de `ConfigurationsScreen.web.tsx`/linha de base de `useScreenStyles`. O Chrome DevTools MCP permanece indisponível para Core Web Vitals.

- Conversão do cadastro de investimento para Web em 2026-09-19: `app/web/add-finance.tsx` passa a resolver `screens/web/AddFinanceScreen.web.tsx`. A nova composição preserva o cadastro CDI, valores em centavos, consulta de bancos/saldo, bloqueio por saldo insuficiente e comportamento pós-registro da variante mobile; no navegador, usa wallpaper, `Grainient`, `StrokeText`, ilustração animada, labels acessíveis e a grade responsiva dos formulários financeiros. `npm run typecheck` passou; `npm run lint:styles` permanece limitado à dívida preexistente de `screens/web/ConfigurationsScreen.web.tsx` e à linha de base de `useScreenStyles`. Vault alinhado em [[Investimentos]], [[Versão Web]] e [[Auditoria de Design]].

- Correção da rail Web ao abrir Configurações em 2026-09-19: links internos de `StaggeredMenu.jsx` agora executam `closeMenu()` antes de navegar. Antes, o `router` podia desmontar o menu ainda expandido e o estado global da rail fazia o próximo navigator — especialmente ao entrar em `/web/home?tab=2` — reaparecer aberto. Cmd/Ctrl+clique e abertura em nova aba permanecem sem interceptação. Vault alinhado em [[Navegação]].

- Correção do warning DOM na carteira Web em 2026-09-19: os nomes de investimento e banco em `FinancialListScreen.web.tsx` usam `isTruncated` no `Text` Web, preservando o truncamento de uma linha por CSS sem encaminhar `numberOfLines` ao `<span>`. Dados, cálculos em centavos, privacidade e fluxos de investimento permanecem inalterados. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` continua limitado à dívida preexistente em `screens/web/ConfigurationsScreen.web.tsx` e na linha de base de `useScreenStyles`. O Chrome DevTools MCP permanece indisponível para Core Web Vitals. Vault alinhado em [[Versão Web]].

- Cabeçalho da timeline de investimentos Web alinhado em 2026-09-19: `FinancialListScreen.web.tsx` agora ancora identidade à esquerda e valor, liquidez e chevron à direita, na mesma composição do cabeçalho de `MandatoryExpensesListScreen.web.tsx`. A expansão, os dados financeiros e as ações permanecem inalterados. `npm run typecheck` e `git diff --check` passaram; `npm run lint:styles` continua limitado à dívida preexistente de `ConfigurationsScreen.web.tsx` e da linha de base de `useScreenStyles`.

- Modais Web da carteira em 2026-09-19: os oito diálogos de `screens/web/FinancialListScreen.web.tsx` usam `Modal size="md"` e o limite responsivo compartilhado de até 510px, removendo o `max-w-[360px]` local que comprimia os formulários no navegador. A variante mobile e a lógica de CDI, edição, aporte, resgate, sincronização e exclusão permanecem inalteradas. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; o Chrome DevTools MCP continua indisponível para Core Web Vitals. Vault alinhado em [[Investimentos]], [[Versão Web]] e [[Auditoria de Design]].

- Listagem da carteira Web alinhada em 2026-09-19: o detalhe expansível de `screens/web/FinancialListScreen.web.tsx` passou a consumir a mesma grade semântica e o grupo de ações com quebra responsiva da timeline de gastos obrigatórios. Dados, cálculos em centavos, privacidade, Firebase e os fluxos de aporte/resgate/sincronização/edição/exclusão permanecem inalterados. Validação: `npm run typecheck` e `git diff --check` passaram; `npm run lint:styles` continua bloqueado exclusivamente por dívida pré-existente em `screens/web/ConfigurationsScreen.web.tsx`. Vault alinhado em [[Investimentos]] e [[Versão Web]].

- Label do gráfico de evolução da carteira em 2026-09-19: `FinancialListScreen.web.tsx` segue o padrão de labels utilitários de `MandatoryExpensesListScreen.web.tsx` (`helperText`, caixa alta e `mt-1`), mantendo o gráfico sem card, título descritivo ou subtítulo externo. Séries, legenda, privacidade, dados e ações Firebase permanecem. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` continua limitado à dívida preexistente em `screens/web/ConfigurationsScreen.web.tsx` e na linha de base de `useScreenStyles`. Vault alinhado em [[Investimentos]], [[Versão Web]] e [[Auditoria de Design]].

- Layout vertical da carteira Web em 2026-09-18: `screens/web/FinancialListScreen.web.tsx` mantém os quatro indicadores da visão consolidada — incluindo `Próximo dia` — e as tabs de rentabilidade no bloco superior; o gráfico de evolução agora ocupa uma linha exclusiva abaixo deles, em vez de dividir a coluna lateral no desktop. Na Web, a moldura externa, o título e o subtítulo do gráfico foram removidos; séries, legenda, privacidade, dados e ações Firebase permanecem. A validação estática deve incluir `npm run lint:styles`, `npm run typecheck`, `npm run web:export` e `git diff --check`; o Chrome DevTools MCP continua indisponível para Core Web Vitals. Vault alinhado em [[Investimentos]], [[Versão Web]] e [[Auditoria de Design]].

- Carteira Web padronizada em 2026-09-18: `screens/web/FinancialListScreen.web.tsx` mantém Firebase, privacidade, centavos e os fluxos de CDI/aporte/resgate/sincronização/edição/exclusão, mas agora usa a timeline Web com foco, alvo de toque, `AnimatedContent` e `Grainient`, além dos labels de formulário `WEB_EXPENSE_CLASS_NAMES`. A rolagem externa e a `sheet` agora seguem os cadastros Web: o hero absoluto termina diretamente no conteúdo, sem reservar a altura residual de um `ScrollView` interno; o conteúdo recebe `pt-4`, alinhado aos 16px usados no encaixe da lista Web de ganhos obrigatórios, para separar o título do wallpaper sem excesso. O gráfico Mantine configura a cor do eixo no slot `styles.root`, sem encaminhar `textColor` ao DOM. `npm run typecheck`, `npm run web:export` e `git diff --check` passaram; `npm run lint:styles` continua bloqueado apenas por dívida pré-existente em `screens/web/ConfigurationsScreen.web.tsx`. Vault alinhado em [[Investimentos]], [[Versão Web]] e [[Auditoria de Design]].

- Configurações Web e skeleton em 2026-09-18: `screens/web/ConfigurationsScreen.web.tsx` passou a ser selecionada em `/web/home?tab=2`, em vez da composição móvel. A variante Web preserva ações, permissões e dados, mas não monta o navigator inferior nativo. A checagem de `adminUser` não bloqueia mais a central inteira; somente a seção administrativa depende dela. `npm run lint:styles` acusa dívida visual da nova variante; após a correção da carteira, `npm run typecheck` passa. Vault alinhado em [[Configurações]], [[Versão Web]] e [[Auditoria de Design]].
...
- Parcelamento por valor total e quitação parcial em 2026-09-02: os formulários obrigatórios de despesas e ganhos, nativos e Web, agora recebem valor total + quantidade de parcelas, persistem `installmentTotalValueInCents`, calculam o valor mensal bloqueado e destinam eventuais centavos residuais à última parcela. Ao ativar o parcelamento, o valor já informado passa a ser o total contratado e não é multiplicado pela quantidade. As listagens permitem escolher quantas parcelas quitar ou receber em um único lançamento real; as transações Firestore avançam apenas a quantidade escolhida, preservam o template até a parcela final e registram o total para reversão consistente. Os antigos controles mensais foram removidos das telas de cadastro/edição. TypeScript validado; o Jest local não pôde localizar os testes através da ponte de caminhos Windows/WSL. Vault alinhado em [[Despesas Fixas]], [[Receitas Fixas]] e [[Transações de Despesas]].
> Atualizado em 2026-09-02.

- Refinamento visual das Tabs da análise por categoria na Web em 2026-09-02: `CategoryAnalysisScreen.web.tsx` mantém `Tabs` controladas do Mantine dentro de um card `notTintedCardClassName` sem preenchimento navy, com a lista e os gatilhos sem bordas próprias, variável de cor explícita para impedir o azul primário padrão do Mantine, amarelo brilhante `#FFEB31` no estado ativo, rótulos na fonte padrão do sistema e centralizados em cada tab, texto/ícone brancos quando selecionados, hover/foco visíveis e desabilitação contextual. **Gastos** e **Ganhos** continuam em ordem fixa no primeiro bloco visual, com o seletor de categorias imediatamente abaixo; estados, relatórios carregados, privacidade e a composição nativa com `components/ui/tabs` permanecem inalterados. Vault alinhado em [[Análise por Categoria]], [[Componentes UI]] e [[Versão Web]].
- Blocos da análise por categoria alinhados à [[Dashboard Home]] em 2026-09-02: `screens/mobile/CategoryAnalysisScreen.tsx` e `screens/web/CategoryAnalysisScreen.web.tsx` agora apresentam evolução, distribuição por conta e últimas movimentações sem card externo, com títulos em caixa alta no padrão da Home e sem ícones decorativos de seção. Gráficos, lista de gastos/ganhos, estados vazios, ícones individuais das movimentações, privacidade e regras de negócio permanecem inalterados. Vault alinhado em [[Análise por Categoria]], [[Componentes UI]] e [[Versão Web]].

- Padronização da largura útil das telas Web em 2026-09-01: `WEB_DASHBOARD_CLASS_NAMES` passou a centralizar o sheet responsivo, o frame central de até `1180px` e o padding lateral de `24px`/`32px` em `contentFrame`/`contentPadding`. As composições Web de formulários, relatórios, configurações, listas e as telas canônicas ainda usadas por rotas Web passaram a consumir o mesmo contrato; Android/iOS e a lógica financeira permanecem inalterados. Vault alinhado em [[Versão Web]], [[Hooks Customizados]] e [[Componentes UI]].

- Acesso direto aos movimentos bancários em 2026-09-01: a opção **Movimentos do banco** passou a ser permanente no grupo Home dos navigators Web e Android/iOS. `components/web/navigation/navigator.web.tsx` entrega o item ao `StaggeredMenu`, enquanto `components/uiverse/navigation/navigator.tsx` reexporta o navigator nativo; a tela `BankMovementsScreen.tsx` e a rota `/web/bank-movements`/`/mobile/bank-movements` permanecem inalteradas. Vault alinhado em [[Navegação]] e [[Gerenciamento de Bancos]].

- Tema claro da navegação Web atualizado em 2026-09-01: `StaggeredMenu` agora recebe `themeMode` e aplica uma variante white/slate com camadas amarelo suave, toggle escuro, estados ativos/foco contrastantes e perfil claro; `navigator.web.tsx` escolhe a paleta correspondente sem alterar a rail, as rotas ou a animação do modo escuro. Vault alinhado em [[Navegação]], [[Sistema de Temas]] e [[Componentes UI]].

- Composição visual Web da carteira, relatórios e configurações de tela em 2026-09-01: `FinancialListScreen.web.tsx`, `CategoryAnalysisScreen.web.tsx`, `FinancialForecastScreen.web.tsx` e `ScreenSettingsScreen.web.tsx` usam o `WebScreenHero` para montar wallpaper de largura total, `Grainient`, `StrokeText` e entrada animada das ilustrações SVG; os sheets seguem acima do hero para manter todos os controles interativos. A lógica Firebase, valores em centavos, privacidade, preferências e rotas mobile permanecem inalteradas. Vault alinhado em [[Versão Web]], [[Navegação]], [[Configurações]], [[Comportamento Pós-Registro]] e [[Visibilidade de Rotas]].

- Organização física das rotas por plataforma em 2026-08-30: a raiz `app/` mantém somente `_layout.tsx`, o entrypoint `/` e o guia; `app/web/` concentra os adaptadores do navegador e `app/mobile/` concentra os adaptadores Android/iOS. As URLs passaram a usar `/web/...` e `/mobile/...`, com `APP_ROUTE_PATHS` selecionando o prefixo conforme `Platform.OS`; o fallback de cada variante permanece dentro do diretório correspondente. Vault alinhado em [[Navegação]], [[Versão Web]], [[Autenticação]] e [[Organização do Código]].
- Correção de resolução dos assets da Home Web em 2026-08-30: `screens/web/HomeScreen.web.tsx` agora sobe dois níveis até `assets/` para importar `wallpaper01.png` e `homeScreen.svg`, evitando a referência inexistente a `screens/assets/` durante o bundle Web. A composição visual e a variante mobile permanecem inalteradas.

- Organização física das telas por plataforma em 2026-08-30: `screens/mobile/` concentra as telas canônicas de Android/iOS e os fluxos sem composição Web dedicada; `screens/web/` concentra as composições exclusivas do navegador. As rotas continuam em `app/`, sem segundo registro, e os adaptadores Web/nativos apontam explicitamente para a pasta correspondente. `/home` usa `HomeTabsScreen.web.tsx` para selecionar as composições Web de Home/Controle e `HomeTabsScreen.tsx` para as telas mobile, mantendo configurações compartilhadas e o mesmo contrato de abas. Vault alinhado em [[Navegação]], [[Versão Web]], [[Componentes UI]] e [[Organização do Código]].

- Organização física dos componentes por plataforma em 2026-08-30: as implementações com divergência de API ou interação foram separadas em `components/web/<sistema>/` e `components/mobile/<sistema>/`. `components/uiverse/<sistema>/` mantém apenas os adaptadores lógicos `.web`/`.native` e o fallback mobile, preservando os imports sem extensão e a seleção automática do Expo. `HomeTabsScreen.web.tsx` também deixou de importar `BackHandler`, mantendo APIs de hardware exclusivamente na implementação mobile. Vault alinhado em [[Componentes por Sistema]], [[Componentes UI]], [[Organização do Código]] e [[Navegação]].

- Conversão do cadastro de ganhos obrigatórios para Web em 2026-08-30: `app/add-mandatory-gains.web.tsx` seleciona explicitamente `screens/web/AddMandatoryGainsScreen.web.tsx`, que replica a composição hero/sheet responsiva de `AddMandatoryExpensesScreen.web.tsx`, com calendário customizado de parcelas, acordeões, controle mensal e configuração de lembretes cumulativos de 1/2/3 dias com aviso opcional no recebimento. A variante usa exclusivamente `getMandatoryGainFirebase`, `addMandatoryGainFirebase`, `updateMandatoryGainFirebase`, `lastReceipt*` e `scheduleMandatoryGainNotification`, preservando valores em centavos, privacidade, navegação e a variante nativa em `screens/mobile/AddMandatoryGainsScreen.tsx`. Vault alinhado em [[Receitas Fixas]], [[Versão Web]], [[Componentes UI]] e [[Firebase Config]].

- Conversão da lista de ganhos obrigatórios para Web em 2026-08-28: `app/mandatory-gains.web.tsx` seleciona explicitamente `MandatoryGainsListScreen.web.tsx`, que replica a composição Web de despesas com calendário mensal, pull-to-refresh, resumo, timeline expansível, confirmação de ações e impressão; o carregamento usa exclusivamente `getMandatoryGainsWithRelationsFirebase`, `lastReceipt*` e `syncMandatoryGainNotifications`, preservando valores em centavos, privacidade e a ausência de quitação antecipada para ganhos. `app/mandatory-gains.native.tsx` mantém `MandatoryGainsListScreen.tsx` no Android/iOS. Vault alinhado em [[Receitas Fixas]], [[Versão Web]], [[Componentes UI]] e [[Firebase Config]].

- Gráficos da lista de ganhos obrigatórios em 2026-08-30: `MandatoryGainsListScreen.web.tsx` agora exibe barra de progresso do ciclo, radar por categoria e dispersão dos dias de recebimento, reutilizando os componentes Mantine DOM da lista de despesas com rótulos de ganhos. Os dados são derivados do loader de ganhos e dos valores exibidos em centavos; `useValueVisibility()` neutraliza os valores do bullet/radar quando a privacidade está ativa. Vault alinhado em [[Receitas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Confiabilidade do resumo de bancos em 2026-08-28: `BankFirebase.ts` limita a três as leituras simultâneas dos snapshots indexados de `monthlyBalances`, preservando o fallback escopado por `personId` quando o índice composto não estiver disponível. `HomeFirebase.ts` trata falhas de histórico e heatmap como dados auxiliares vazios, sem derrubar os cartões de banco, saldos, totais do mês ou Dinheiro. Vault alinhado em [[Dashboard Home]], [[Gerenciamento de Bancos]], [[Balanço Mensal]] e [[Hooks Customizados]].

- Padronização dos formulários Web em 2026-08-27: `AddRegisterExpensesScreen.web.tsx`, `AddRegisterGainScreen.web.tsx`, `AddMandatoryExpensesScreen.web.tsx`, `AddRegisterMonthlyBalanceScreen.web.tsx`, `TransferScreen.web.tsx`, `AddRescueScreen.web.tsx`, `AddRegisterUserScreen.web.tsx`, `AddRegisterTagScreen.web.tsx` e `AddUserRelationScreen.web.tsx` usam labels com `WEB_EXPENSE_CLASS_NAMES.fieldLabel`, espaçamento `mb-2` e linhas `sectionLabel` para alinhar ícones de informação aos títulos. As variantes Web específicas reutilizam a lógica Firebase das telas nativas sem alterar regras de centavos, persistência ou navegação. Vault alinhado em [[Versão Web]], [[Componentes UI]], [[Balanço Mensal]], [[Transferências]], [[Resgate de Caixa]], [[Gerenciamento de Usuários]] e [[Gerenciamento de Tags]].

- Select Web reutilizável no cadastro de gastos obrigatórios em 2026-08-27: `web-select-field.tsx` centraliza trigger, menu, valor selecionado, seta, hover, seleção, foco de teclado e espaçamento nos tokens de `useScreenStyles()`. `AddMandatoryExpensesScreen.web.tsx` usa o componente para “Começar a lembrar”, removendo o menu nativo cinza e mantendo a lista no fluxo vertical sem sobrepor “Avisar também no vencimento” ou “Horário preferido”. Vault alinhado em [[Componentes UI]], [[Despesas Fixas]] e [[Versão Web]].

- Seletor Web de horário alinhado ao select em 2026-08-27: `time-picker-field.web.tsx` deixou de depender do menu nativo de `input type="time"` e agora mostra trigger com padding/seta, valor escolhido, duas listas roláveis de hora e minuto, hover, seleção e teclado. Os tokens ficaram em `webTimePickerClassNames` dentro de `useScreenStyles()`; Android/iOS continuam com o seletor nativo. Vault alinhado em [[Componentes UI]], [[Despesas Fixas]], [[Hooks Customizados]] e [[Versão Web]].

- Alinhamento do label de observações no cadastro Web em 2026-08-27: `AddMandatoryExpensesScreen.web.tsx` neutraliza a margem inferior estrutural do `fieldLabel` quando ele divide a linha com o ícone de informação, mantém o popover inline no mesmo eixo vertical do texto e reaplica o espaçamento entre a linha do label e o campo no contêiner da seção. O comportamento do formulário e os estilos das demais telas permanecem inalterados. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Switches Web padronizados em 2026-08-27: `useScreenStyles()` expõe `switchActiveThumbColor` em amarelo âmbar `#EAB308` para a bolinha ativa dos switches de `AddMandatoryExpensesScreen.web.tsx`. O track, a bolinha inativa, callbacks e persistência permanecem inalterados. Vault alinhado em [[Componentes UI]], [[Sistema de Temas]] e [[Hooks Customizados]].

- Cadastro Web de gastos obrigatórios refinado em 2026-08-27: `AddMandatoryExpensesScreen.web.tsx` mantém o shell de `AddRegisterExpensesScreen.web.tsx` (wallpaper/`Grainient`/`StrokeText`, sheet sobreposto, grid responsivo, foco e `Navigator`), mas prioriza nome, valor mensal, vencimento, observações e categoria. O input de vencimento permanece fixo; o acordeão **Mais opções do vencimento** concentra somente o switch **Contar por dia útil**. Parcelamento e lembrete ficam no acordeão **Mais opções**, que abre quando já há configuração ativa; a quantidade de parcelas usa o `NumberInput` Mantine com a mesma superfície do `Select` padrão; o seletor de antecedência do lembrete usa lista Web própria no fluxo para não expor o menu nativo cinza do navegador; o controle mensal aparece somente para um template salvo. A lógica Firebase, os valores em centavos e a variante nativa permanecem intactos. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Radar e Scatter Web dos gastos obrigatórios em 2026-08-24: `MandatoryExpensesListScreen.web.tsx` agora exibe abaixo do bullet mensal dois cards responsivos lado a lado. O `RadarChart` agrega os valores exibidos por categoria, converte cents para reais na camada visual e oculta a escala numérica radial; o `ScatterChart` cruza o `resolvedDueDate` do ciclo atual entre dia de semana e dia do mês, com séries de pendentes e pagos/concluídos, mantendo a mesma distribuição exibida no calendário. Os gráficos usam componentes Expo DOM serializáveis, mantêm o tema claro/escuro e neutralizam o Radar quando **Ocultar valores** está ativa. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Timeline Web dos gastos obrigatórios alinhada à Home em 2026-08-24: `MandatoryExpensesListScreen.web.tsx` voltou a renderizar a linha compacta com trilho, ícone, nome/categoria, valor/vencimento e seta; o detalhe usa `AnimatedContent` com saída antes do unmount e `Grainient` contextual, preservando registrar, editar, quitar parcelas, reivindicar e excluir. Itens pendentes usam gradiente laranja/âmbar e itens pagos ou concluídos usam verde; a variante mobile e as regras financeiras permanecem inalteradas. Vault alinhado em [[Despesas Fixas]] e [[Versão Web]].

- Datas explícitas em aportes, resgates e sincronizações em 2026-08-24: `FinancialListScreen.tsx` passou a permitir escolher separadamente a data da sincronização prévia e a data efetiva do aporte/resgate, usando `DatePickerField` no padrão de `AddRegisterExpensesScreen.tsx`. A sincronização manual comum também aceita a data observada. As datas começam no dia atual, aceitam hoje ou datas anteriores, rejeitam amanhã e qualquer data futura, são validadas em `DD/MM/AAAA`, preservam o dia civil com o horário atual na persistência e não alteram a regra de centavos nem a exclusão de `isInvestmentDeposit`/`isInvestmentRedemption` dos totais. Vault alinhado em [[Investimentos]], [[Monitoramento de Investimentos]], [[Transações de Despesas]], [[Transações de Receitas]] e [[Componentes UI]].

- Resumo diário Web do calendário refinado em 2026-08-24: `date-calendar.web.tsx` agora usa a mesma linha compacta da timeline da Home para cada gasto/ganho, ordena os itens do dia com pendentes antes dos concluídos e desempata por nome, mantém o detalhe recolhível em uma superfície contida e preserva registrar, quitar parcelas (somente despesas), editar, reivindicar e excluir. A variante nativa não foi alterada; os contratos de dados, valores em centavos, privacidade e confirmação da listagem permanecem os mesmos. Vault alinhado em [[Despesas Fixas]], [[Receitas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Fundo do detalhe diário Web alinhado à Home em 2026-08-24: `date-calendar.web.tsx` monta `Grainient` somente enquanto o item está expandido, com stops derivados do tom de ganho/despesa e conteúdo textual em camada superior. O fallback CSS do componente mantém a leitura quando WebGL2 não está disponível; a variante nativa e as ações financeiras permanecem inalteradas. Vault alinhado em [[Componentes UI]], [[Versão Web]], [[Despesas Fixas]] e [[Receitas Fixas]].

- Separador visual removido do resumo diário Web em 2026-08-24: os itens de `date-calendar.web.tsx` continuam espaçados, mas não exibem mais uma borda entre gastos/ganhos consecutivos. A navegação mensal do `date-picker.web.tsx` permanece inalterada. Vault alinhado em [[Componentes UI]], [[Despesas Fixas]] e [[Receitas Fixas]].

- Hover do resumo diário Web desativado em 2026-08-24: os botões de ação e a linha clicável que expande o item não alteram mais o fundo ao passar o mouse; foco visível e estados disabled permanecem preservados. Vault alinhado em [[Componentes UI]].

- Valor previsto removido do detalhe expandido Web em 2026-08-24: o card Grainient mantém status e metadados, enquanto o valor continua somente na linha principal do item para evitar repetição visual. A variante nativa permanece inalterada. Vault alinhado em [[Componentes UI]], [[Despesas Fixas]] e [[Receitas Fixas]].

- Animação do card do resumo diário Web adicionada em 2026-08-24: `date-calendar.web.tsx` agora monta o detalhe com `AnimatedContent`, usando entrada curta por deslocamento/opacidade e saída antes do unmount, como na Home e na listagem de gastos obrigatórios. A redução de movimento, os dados e as ações financeiras permanecem preservados. Vault alinhado em [[Componentes UI]], [[Despesas Fixas]] e [[Receitas Fixas]].

- Alinhamento da linha do resumo diário Web em 2026-08-24: o recuo horizontal interno foi removido da linha clicável, deixando o ícone alinhado ao início do conteúdo do modal sem alterar a área vertical, o foco ou o valor/data. Vault alinhado em [[Componentes UI]].

- Ação de exclusão do resumo diário Web em branco em 2026-08-24: o botão, o ícone e o texto **Excluir** usam branco dentro do card Grainient, mantendo foco visível e a confirmação da ação. Vault alinhado em [[Componentes UI]].

- Isolamento de componentes Web/mobile em 2026-08-23: os componentes reutilizados por telas nativas e Web agora usam resolução automática no mesmo caminho lógico, com implementações `.native.tsx` e `.web.tsx` independentes para navegação/notifier, `date-picker`, `date-calendar`, seletores de banco/categoria, disponibilidade de categoria, `bank-card-surface`, `loader`, `screen-dismiss-keyboard`, `web-app-shell` e os cards/boundary do Assistente. Os contratos públicos, formatos `DD/MM/AAAA`/`HH:MM`, valores em centavos, privacidade e ações de confirmação foram preservados; o Web do assistente usa Mantine e o mobile mantém gifted-charts. Gráficos/editor que começam com `'use dom'` continuam deliberadamente na fronteira Expo DOM. Vault alinhado em [[Componentes por Sistema]], [[Componentes UI]], [[Organização do Código]], [[Versão Web]], [[Despesas Fixas]], [[Receitas Fixas]] e [[Assistente Lumus]].

- Organização por sistema em 2026-08-23: os componentes customizados foram separados nas pastas de domínio de `components/uiverse/` (`navigation`, `shared`, `feedback`, `banks`, `categories`, `recurring`, `dashboard`, `reports`, `investments`, `annotations` e `assistant`). Os componentes DOM Web de infraestrutura foram agrupados em `components/web/motion`, `components/web/navigation` e `components/web/visuals`. Todas as telas e testes passaram a importar os caminhos físicos correspondentes; `components/ui/`, rotas, regras financeiras, persistência e contratos Web permaneceram inalterados. Vault alinhado em [[Componentes por Sistema]], [[Componentes UI]], [[Organização do Código]] e [[Versão Web]].

- Paridade mobile do resumo diário Web em 2026-08-23: `date-calendar.tsx` removeu a timeline exclusiva do navegador e reutiliza para cada item a mesma linha de Android/iOS — ícone, nome/categoria, valor/data e seta. O nome continua normalizado, com truncamento seguro e fallback acessível apenas para dados vazios. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Bullet de progresso de pagamentos obrigatórios na Web em 2026-08-21: `MandatoryExpensesListScreen.web.tsx` agora exibe somente `mandatory-expense-payment-bullet-chart.tsx` abaixo do resumo mensal. O limite usa o total de referência do ciclo e o preenchimento usa o total pago, mantendo centavos na origem e sem repetir título, contagem ou soma paga/total. Quando **Ocultar valores** está ativa, a Web envia somente valores neutros e não revela a proporção. O Expo DOM Component usa o `BulletChart` oficial de `@mantine/charts@9.5.1`, com `@mantine/core`, `@mantine/hooks` e React/React DOM alinhados às versões compatíveis exigidas pela biblioteca. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Compactação do resumo diário de despesas obrigatórias na Web em 2026-08-21: `date-calendar.tsx` impede que o corpo do modal e o card expandido cresçam para preencher o espaço livre do diálogo. A superfície passa a manter altura intrínseca, com rolagem somente quando o conteúdo excede a altura útil; o item expansível também expõe seu estado para acessibilidade. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].


- Correção de montagem do resumo diário Web em 2026-08-21: `date-calendar.tsx` deixou de enviar arrays de estilo aos limites DOM de `ModalContent`, `ModalBody` e `Text`, evitando que React tente escrever o índice `0` em `CSSStyleDeclaration` ao abrir a tela. A apresentação e a lógica do modal permanecem as mesmas. Vault alinhado em [[Versão Web]] e [[Componentes UI]].

- Automação de versão em 2026-08-21: `script/update_app_version.js`, executado por `npm run version:set -- <versão>`, sincroniza a versão SemVer em `app.json`, `package.json`, `screens/mobile/LoginScreen.tsx` e `screens/web/LoginScreen.web.tsx`. O script falha sem alterar arquivos quando a versão é inválida ou quando algum texto de versão esperado não é encontrado.

- Controle de acesso e preferências em Configurações em 2026-08-21: `ConfigurationsScreen.tsx` oculta completamente o accordion de usuários para contas sem `adminUser`, mantém bancos, categorias e vínculos disponíveis para usuários padrão e reúne **Ocultar valores** e **Confiar neste dispositivo** em um único card da preferência financeira. Vault alinhado em [[Configurações]], [[Gerenciamento de Usuários]], [[Gerenciamento de Bancos]] e [[Gerenciamento de Tags]].

- Ajuste do seed local em 2026-08-21: `backend/scripts/seed.ts` grava o usuário demo com `adminUser: true`, mantendo a conta `usuario@demo.lumus.local` apta a testar os fluxos administrativos no Firebase Emulator. Vault alinhado em [[Firebase Config]] e [[Gerenciamento de Usuários]].

- Correção de produção em 2026-08-21: o resumo de bancos falhava na leitura do saldo legado porque o índice composto de `monthlyBalances` (`bankId` + `personId` + `year` + `month`) estava versionado em `firestore.indexes.json`, mas ausente do projeto Firebase `finances-app-e8685`. O índice foi publicado sem remover índices existentes e a leitura em `functions/BankFirebase.ts` ganhou fallback escopado por `personId` enquanto o índice termina de ser construído. A nova build Android usa o perfil `production-apk`; vault alinhado em [[Dashboard Home]], [[Gerenciamento de Bancos]], [[Balanço Mensal]] e [[Cache e Leituras Firebase]].

- Correção do carregamento do portfólio da Home em 2026-08-21: `firestore.rules` passou a declarar `investmentCdiRates` com leitura para o usuário/relacionados e escrita somente do proprietário antes do cutover. `HomeFirebase.ts` trata o CDI como dependência opcional e mantém os valores-base quando essa leitura falha, permitindo que os gráficos Web e Android/iOS carreguem a carteira. Testes de Rules cobrem leitura por relacionado, consulta por `personId`, escrita do dono e bloqueio de usuário externo. Vault alinhado em [[Dashboard Home]], [[Investimentos]] e [[Firebase Config]].

- Correção do desenvolvimento local em 2026-08-20: `firebase.json` faz Auth, Firestore e Functions escutarem em `0.0.0.0` para dispositivos físicos; `script/build_and_run.sh` detecta IP LAN, rejeita Suites antigas presas em localhost e força `expo start --go --lan` no fluxo `npm run dev:local`. `npm run dev:local:web` inicia o mesmo alvo no navegador, e o seed imprime as credenciais da conta demo local. Android Emulator continua usando `adb reverse`; o modo `--dev-client` permanece separado. Vault alinhado em [[Firebase Config]].

- Removida em 2026-08-20 a animação de entrada vertical do formulário Web de `AddRegisterGainScreen.web.tsx` e `AddRegisterExpensesScreen.web.tsx`. A animação da ilustração no hero permanece; somente o wrapper `AnimatedContent` da superfície do formulário foi removido. Vault alinhado em [[Versão Web]].

- Organização estrutural em 2026-08-20: `app/_layout.tsx` passou a ser o bootstrap mínimo e delega providers, guard autenticado e ciclo de notificações a `components/app/app-root.tsx`; `app/home.tsx` delega o container de abas a `screens/mobile/HomeTabsScreen.tsx`. A sidebar desktop morta saiu do navigator nativo, e Web/nativo compartilham a rotina segura de saída em `utils/secureLogout.ts`. Nove telas administrativas de baixa divergência voltaram a ter um único `.tsx` com adaptação `web:`/componentes resolvidos por plataforma, removendo cópias `.web.tsx` quase idênticas. O `tsconfig` ignora `dist/`, evitando que bundles gerados sejam analisados como fonte; `npm run typecheck`, `typecheck:backend` e `check` formalizam a verificação local. Vault alinhado em [[Organização do Código]], [[Navegação]], [[Versão Web]] e [[Componentes UI]].

- Correção do bootstrap Firebase Android em 2026-08-20: `utils/firebaseRuntime.ts` captura cada `EXPO_PUBLIC_*` por acesso direto para o Metro incorporar a configuração no bundle, e releases locais com credenciais completas inferem produção sem depender de uma variável de alvo ausente. Somente `development` usa o Emulator para os dados financeiros; `preview` passa a ser APK instalável contra produção com App Check debug, enquanto `production`/`production-apk` mantêm Play Integrity. Todos os perfis EAS Android passaram a exigir `GOOGLE_SERVICES_JSON` após a ponte híbrida de IA de 2026-09-21; a ação **Run Dev Client** inicia/semeia a Suite local e aplica `adb reverse`. Isso elimina a exceção anterior à montagem do Expo Router que mantinha `splash-icon.png` indefinidamente. Vault alinhado em [[Firebase Config]], [[Autenticação]] e [[Navegação]].

- Correção do espaçamento no resumo diário Web em 2026-08-19: o `ModalBody` do `DateCalendar` não cresce mais até o `maxHeight` do diálogo no navegador; ele se ajusta ao conteúdo e mantém rolagem somente quando a lista excede a altura útil. O layout deixa de distribuir grandes vazios entre os gastos do mesmo dia. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Portabilidade e escala dos modais da listagem Web de despesas fixas em 2026-08-19: `components/ui/modal/index.tsx` usa `lg` como tamanho padrão, a confirmação de ação em `MandatoryExpensesListScreen.web.tsx` remove o teto artificial de 520 px e o resumo diário do `DateCalendar` recebe `modalSize="lg"` somente nessa composição Web. O corpo modal contém o overscroll no navegador e os fechamentos têm rótulo acessível; os diálogos nativos compactos permanecem com seus limites explícitos. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].

- Remoção do botão dedicado de atualizar da listagem Web de despesas fixas em 2026-08-19: `MandatoryExpensesListScreen.web.tsx` mantém o recarregamento por pull-to-refresh, mas não exibe mais a ação `Atualizar`. Vault alinhado em [[Despesas Fixas]].

- Ajuste de espaçamento e altura visual do hero da lista Web de despesas fixas em 2026-08-19: `MandatoryExpensesListScreen.web.tsx` usa as classes estruturais compartilhadas do hero, com recorte de overflow e imagem posicionada em toda a área. Como as telas Web com sheet sobreposto ocultam os 64 px finais do `heroHeight`, a lista usa `heroHeight - 64` para manter o mesmo wallpaper visível e inicia o wrapper do calendário 16 px depois dele. O `VStack` que contém o `DateCalendar` não reserva margem superior adicional. Vault alinhado em [[Despesas Fixas]] e [[Versão Web]].

- Conversão das despesas obrigatórias para Web em 2026-08-18: `AddMandatoryExpensesScreen.web.tsx` e `MandatoryExpensesListScreen.web.tsx` passaram a concentrar composições Web independentes, mantendo a lógica Firebase, os valores em centavos e as rotas canônicas. O cadastro usa calendário modal customizado, horário nativo do navegador e controle mensal; a listagem usa `DateCalendar`, resumo, timeline, atualização manual, modais e impressão do PDF. O navegador preserva a configuração de lembretes, mas não agenda notificações. Vault alinhado em [[Despesas Fixas]], [[Versão Web]] e [[Componentes UI]].

- A conversão inicial para variantes Web em 2026-08-18 confirmou que nenhum arquivo de `app/` está órfão: cada entrada é uma rota de `APP_ROUTE_PATHS`, do guard central ou uma compatibilidade legada documentada. Em 2026-08-20, as nove variantes de baixa divergência foram consolidadas novamente nos arquivos canônicos; lógica Firebase, valores em centavos, navegação e comportamento pós-submit continuam compartilhados por plataforma. Vault alinhado em [[Organização do Código]], [[Versão Web]] e [[Navegação]].
...
- Refinamento visual das telas Web em 2026-08-18: `components/web/navigation/web-screen-hero.web.tsx` e seu adaptador lógico centralizam wallpaper, `Grainient`, título `StrokeText` e entrada animada das ilustrações SVG nas nove telas convertidas. Android/iOS continuam usando `components/mobile/navigation/web-screen-hero.native.tsx`; a borda externa adicionada à superfície dos formulários foi removida. Vault alinhado em [[Versão Web]] e [[Componentes UI]].
- Atualizado em 2026-08-18.
...
- Conversão do cadastro de ganhos para Web em 2026-08-17: `screens/web/AddRegisterGainScreen.web.tsx` passou a reutilizar o shell hero/sheet, wallpaper, `Grainient`, `StrokeText`, `AnimatedContent`, rolagem única, grade responsiva e ActionSheets estabelecidos por `AddRegisterExpensesScreen.web.tsx`. A lógica nativa de ganhos permanece responsável por centavos, formatos de recebimento, categorias, templates/ganhos obrigatórios, resgates de investimento e comportamento pós-submit; Android/iOS continuam usando `AddRegisterGainScreen.tsx`. Vault alinhado em [[Versão Web]], [[Transações de Receitas]] e [[Componentes UI]].
...
- Animação do alerta Web em 2026-08-13: `notifier-alert.web.tsx` envolve o `Alert` Mantine com `components/web/motion/AnimatedContent.jsx`, usando entrada horizontal de 300px pela direita, duração de 1s, `power3.out` e opacidade inicial zero. O wrapper nativo e os fluxos Android/iOS permanecem inalterados. Vault alinhado em [[Notificações]] e [[Componentes UI]].
> Atualizado em 2026-08-13.
...
- Efeito de entrada do título da Home Web em 2026-08-13: `HomeScreen.web.tsx` usa `components/web/visuals/StrokeText.jsx` com `trigger="mount"` para desenhar o título do hero, mantendo a ilustração com `AnimatedContent` e Android/iOS inalterados. Vault alinhado em [[Dashboard Home]] e [[Componentes UI]].
> Atualizado em 2026-08-13.

- Animação da ilustração sobre o wallpaper da Home Web em 2026-08-13: `HomeScreen.web.tsx` envolve `homeScreen.svg` com `components/web/motion/AnimatedContent.jsx`, usando entrada vertical de 300px, duração de 1s, opacidade inicial zero e `power3.out`. O wallpaper permanece estático e Android/iOS permanecem inalterados. Vault alinhado em [[Dashboard Home]] e [[Componentes UI]].
> Atualizado em 2026-08-13.

- Camada visual do wallpaper da Home Web em 2026-08-13: `HomeScreen.web.tsx` mantém `wallpaper01.png` como base e aplica `components/web/visuals/Grainient.jsx` por cima do hero, com paleta adaptada ao tema e granulação sutil. Android/iOS permanecem inalterados. Vault alinhado em [[Dashboard Home]], [[Versão Web]] e [[Componentes UI]].
> Atualizado em 2026-08-13.
...
- Regressão da tela mobile de Login corrigida em 2026-08-12: `screens/mobile/LoginScreen.tsx` foi restaurada integralmente, preservando wallpaper, logos por tema, card sobreposto e comportamento de teclado do Android/iOS. `screens/web/LoginScreen.web.tsx` permanece a implementação exclusiva do navegador; não há terceiro arquivo `.native.tsx`. Vault alinhado em [[Autenticação]], [[Componentes UI]] e [[Sistema de Temas]].
> Atualizado em 2026-08-12.
...
- Ajuste contextual no `components/uiverse/navigation/navigator.tsx`: em `/bank-movements`, o grupo Home passa a exibir **Início**, **Movimentos do banco** e **Análise por Categoria**, mantendo a tela de movimentos indicada e preservando o retorno explícito para `/home?tab=0`; vault alinhado em [[Navegação]] e [[Gerenciamento de Bancos]].
...
- Ajuste visual em `screens/mobile/ConfigurationsScreen.tsx`: o filtro de categorias passou a usar `fieldContainerCardClassName`, no mesmo padrão de categorias das telas de registro, para acomodar ícone e helper dentro do card; vault alinhado em [[Configurações]].
