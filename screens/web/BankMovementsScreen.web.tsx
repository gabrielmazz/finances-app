...
export default function BankMovementsScreen() {
	...
	const bankMovementsSheetClassName = webDashboardClassNames.webSheetCompact;
	...
	// carregar o extrato sem exigir um retorno à Home (ver [[Gerenciamento de Bancos]]).
	...
	React.useEffect(() => {
		...
		const availableTagIds = new Set(availableTagFilters.map(option => option.id));
		const nextSelectedTagFilterIds = selectedTagFilterIds.filter(tagId => availableTagIds.has(tagId));
		if (nextSelectedTagFilterIds.length !== selectedTagFilterIds.length) {
			setSelectedTagFilterIds(nextSelectedTagFilterIds);
		}
	}, [availableTagFilters, selectedTagFilterIds]);

	const selectedTagFilterOptions = React.useMemo(
		() => {
			const availableTagsById = new Map(availableTagFilters.map(option => [option.id, option]));
			return selectedTagFilterIds
				.map(tagId => availableTagsById.get(tagId))
				.filter((option): option is AvailableTagFilterOption => option !== undefined);
		},
		[availableTagFilters, selectedTagFilterIds],
	);
	const tagInputData = React.useMemo(
		() => availableTagFilters.map(option => ({ value: option.id, label: option.id })),
		[availableTagFilters],
	);
	const resolveTagFilterId = React.useCallback(
		(value: string) => {
			const normalizedValue = value.trim().toLocaleLowerCase();
			return (
				availableTagFilters.find(
					option => option.id === value || option.label.trim().toLocaleLowerCase() === normalizedValue,
				)?.id ?? null
			);
		},
		[availableTagFilters],
	);
	const tagInputFilter = React.useCallback<OptionsFilter>(
		({ options, search, limit }) => {
			...
			return options
				.filter(option => {
					...
					const tagOption = availableTagFilters.find(item => item.id === option.value);
					return tagOption?.label.trim().toLocaleLowerCase().includes(normalizedSearch);
				})
				.slice(0, limit);
		},
		[availableTagFilters],
	...
	const visibleMovements = React.useMemo(() => {
		...
		const selectedTagIds = new Set(selectedTagFilterIds);
		return movementsMatchingMovementFilter.filter(
	...
	}, [movementFilter, selectedTagFilterOptions]);
